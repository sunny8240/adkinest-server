...
const idOf = (record) => ({ ...record, id: String(record._id) });
const sendList = (res, records) => res.json(records.map((record) => idOf(record)));
...
adminRouter.put("/leads/:id", async (req, res, next) => {
  try {
    ...
    const lead = await Lead.findByIdAndUpdate(id, { status: status }, { new: true, runValidators: true }).lean();
