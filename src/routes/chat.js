...
const messagesFor = (conversationId) => Message.find({ conversationId }).sort({ createdAt: 1 }).lean();
const outputMessages = (messages) => messages.map((message) => ({ ...message, id: String(message._id), conversation_id: String(message.conversationId) }));
